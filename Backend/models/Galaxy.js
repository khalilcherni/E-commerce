...
module.exports={
    ...
    getAll: function (callback) {
        const sql = 'SELECT * FROM `galaxy`'
        connection.query(sql, function (error, results, fields) {
          callback(error, results);
        });
      },
      getOne: function (name,callback) {
        const sql=  'SELECT * FROM `galaxy` WHERE `name`=?'
        ...
        add: function (x,callback) {
          const sql='INSERT INTO `galaxy` SET ?'
          connection.query(sql,x,function(error,results){
            callback(error,results)
          })
        },
         updateproduct : (params, id, callback) => {
            const sql = 'UPDATE galaxy SET `name`=?, `price`=?, `imageUrl`=? WHERE `id`=?';
        ...
        deleteproduct:(id,callback)=>{
            const sql='DELETE FROM galaxy WHERE `id`=?';
